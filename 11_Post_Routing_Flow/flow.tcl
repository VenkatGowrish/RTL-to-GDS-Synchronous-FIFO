...
if { $rcx_rules_file != "" } {
  ...
  write_spef gcd/design.spef

  read_spef gcd/design.spef
} else {
...
report_checks -path_delay min_max -format full_clock_expanded \
    -fields {input_pin slew capacitance} -digits 3 > gcd/checks.rpt

report_worst_slack -min -digits 3 > gcd/worst_slack_min.rpt

report_worst_slack -max -digits 3 > gcd/worst_slack_max.rpt

report_tns -digits 3 > gcd/tns.rpt

report_check_types -max_slew -max_capacitance -max_fanout \
    -violators -digits 3 > gcd/check_types.rpt

report_clock_skew -digits 3 > gcd/clock_skew.rpt

report_power -corner $power_corner > gcd/power.rpt

report_floating_nets -verbose > gcd/floating_nets.rpt

report_design_area > gcd/design_area.rpt

write_verilog gcd/design.v

write_def gcd/design.def
